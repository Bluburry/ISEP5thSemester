...
{
	public class US15IntegrationTest
	{
		...
		public US15IntegrationTest()
		{
			...
			_mockStaffRepo.Setup(r => r.GetStaffByLicense(It.IsAny<LicenseNumber>())).ReturnsAsync(_staff);
		...
		[Fact]
		public async Task Test_US15_FilterStaff1()
		{
			Staff oldStaff = new(
				new LicenseNumber("12345678"),
				new ContactInformation(
			...
			_mockStaffRepo.Setup(r => r.GetFilteredStaff(It.IsAny<QueryDataDto>())).Returns(list);
			
			await _ctrl.FilterStaff(null, null, null, "Specialization", null, _tokenDto.TokenId);

			_mockStaffRepo.Verify(r => r.GetFilteredStaff(It.IsAny<QueryDataDto>()), Times.Once);
		}

		[Fact]
		public async Task Test_US15_FilterStaff2()
		{
			Staff oldStaff = new(
				new LicenseNumber("12345678"),
				new ContactInformation(
			...
			IEnumerable<Staff> list = [newStaff];

			_mockStaffRepo.Setup(r => r.GetFilteredStaff(It.IsAny<QueryDataDto>())).Returns(list);

			await _ctrl.FilterStaff(null, null, "emailTest@hospital.com", "Specialization", null, _tokenDto.TokenId);
