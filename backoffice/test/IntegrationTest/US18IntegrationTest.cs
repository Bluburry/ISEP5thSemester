...
{
    public class US18IntegrationTest
    {
        ...
        [Fact]
        public async Task DeleteOperationRequest_Successful(){
			...
			var result = await controller.DeleteOperationRequest(_opReq.Id.AsString(), _token.ToDto().TokenId);
			...
			_mockRequestRepo.Verify(r => r.GetByIdAsync(It.IsAny<OperationRequestId>()), Times.Never);
		}

        [Fact]
        public async Task DeleteOperationRequest_UnsuccessfulNoRequest(){
			...
			var result = await controller.DeleteOperationRequest("c185d517-d467-4ba5-a789-eb4f77a194b1", _token.ToDto().TokenId);
			Assert.Null(result.Value);

			_mockTokenSvc.Verify(r => r.GetByIdAsync(It.IsAny<TokenId>()), Times.Once);
			_mockRequestRepo.Verify(r => r.GetRequestById(It.IsAny<OperationRequestId>()), Times.Once);
			_mockDoctorRepo.Verify(r => r.GetDoctorByLicenseNumber(It.IsAny<LicenseNumber>()), Times.Never);
			_mockRequestRepo.Verify(r => r.GetByIdAsync(It.IsAny<OperationRequestId>()), Times.Never);
		}
    }
}
