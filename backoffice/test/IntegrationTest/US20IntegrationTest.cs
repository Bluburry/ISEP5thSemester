...
{
	public class US20IntegrationTest
	{
		...
		public async Task RegisterOperationType_Successful()
		{
			...
			// _operationType.RequiredSpecialists = _reqSpec;
			...
			var val = await controller.Create(_operationType.ToDTO(), _token.ToDto().TokenId);
		...
		public async Task RegisterOperationType_UnsuccessfulNotEnoughPhases()
		{
			...
			_operationType.OperationPhases.RemoveAt(0);
			...
			var exception = await Assert.ThrowsAsync<ArgumentException>(async () => await controller.Create(_operationType.ToDTO(), _token.ToDto().TokenId));
