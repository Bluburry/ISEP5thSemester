...
{
	public class US_7_2_15_IntegrationTest
	{
		...
		public US_7_2_15_IntegrationTest()
		{
			_mockUnitOfWork = new();
			_mockSpecRepo = new();
			_mockLogRepo = new();
		...
		[Fact]
		public async Task DeleteSpecialization_Successful()
		{
		...
		[Fact]
		public async Task DeleteSpecialization_UnSuccessful()
		{
			_mockSpecRepo.Setup(s => s.GetByIdAsync(It.IsAny<SpecializationCode>()));//.ReturnsAsync(_spec);
			...
			_mockSpecRepo.Verify(r => r.GetByIdAsync(It.IsAny<SpecializationCode>()), Times.Once);
			_mockSpecRepo.Verify(r => r.Remove(It.IsAny<Specialization>()), Times.Never);
