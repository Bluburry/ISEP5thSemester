...
{
	public class US12IntegrationTest
	{
		...
		[Fact]
		public async Task Test_US12_CreatesDoctor()
		{
			await _ctrl.CreateStaff(_staff.toDto(), _tokenDto.TokenId);

			_mockUserRepo.Verify(r => r.GetByIdAsync(It.IsAny<Username>()), Times.Once);
			_mockStaffRepo.Verify(r => r.GetStaffByContact(It.IsAny<ContactInformation>()), Times.Once);
			...
			_mockDocRepo.Verify(r => r.AddAsync(It.IsAny<Doctor>()), Times.Once);
			_mockWorkUnit.Verify(w => w.CommitAsync(), Times.Once);
		}

		[Fact]
		public async Task Test_US12_DuplicateContactInfo()
		{
			_mockStaffRepo.Setup(r => r.GetStaffByContact(It.IsAny<ContactInformation>())).ReturnsAsync(_staff);

			var exception = await Assert.ThrowsAsync<ArgumentException>(async () => await _ctrl.CreateStaff(_staff.toDto(), _tokenDto.TokenId));

			_mockUserRepo.Verify(r => r.GetByIdAsync(It.IsAny<Username>()), Times.Once);
			_mockStaffRepo.Verify(r => r.GetStaffByContact(It.IsAny<ContactInformation>()), Times.Once);
			_mockStaffRepo.Verify(r => r.AddAsync(It.IsAny<Staff>()), Times.Never);
			_mockSpecializationRepo.Verify(r => r.GetByName(It.IsAny<string>()), Times.Never);
			_mockDocRepo.Verify(r => r.AddAsync(It.IsAny<Doctor>()), Times.Never);
			_mockWorkUnit.Verify(w => w.CommitAsync(), Times.Never);
