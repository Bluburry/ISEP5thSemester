...
staff(d004,anaesthesist,anaesthetist,[so2,so3,so4]).
staff(n001,doctor,instrumenting,[so2,so3,so4]).
staff(n002,doctor,circulating,[so2,so3,so4]).
staff(n003, anaesthesist, anaesthesist, [s02,s03,s04]).
staff(a001,cleaner,medical,[so2,so3,so4]).
...
%surgery(so1,45,20,45).
surgery(so2,15,20,15).
surgery(so3,15,20,15).
surgery(so4,15,25,15).
...
%
initialize:-write('Population size: '),read(PS),
	(retract(population(_));true), asserta(population(PS)),
        write('Number of elites that get passed instantly to the new generation: '),read(E),
        (PS > E,
	(retract(number_passed_elites(_));true), asserta(number_passed_elites(E)),
	...
	(retract(prob_mutation(_));true), asserta(prob_mutation(PM)));
        write('There can not be more passed elites than members in the population.\n'), !, initialize.

initialize_aditional_termination_params :-
    % Optional input for goal fitness value
    write('Goal fitness value (-1 if not): '), read(FV),
    (FV \= -1 ->
        (retract(goal_fitness_value(_)); true), asserta(goal_fitness_value(FV))
    ; true),

    % Optional input for generations until stabilization
    write('Number of equal generations until stabilized (-1 if not): '), read(SG),
    (SG \= -1 ->
        (retract(generations_until_stabilization(_)); true), asserta(generations_until_stabilization(SG)),
        (retract(current_generations_until_stabilization(_)); true), asserta(current_generations_until_stabilization(SG))
    ; true),
    write('Maximum duration of genetic algorithm (-1 if not): '), read(MT),
    (MT \= -1 ->
        (retract(end_time(_)); true),get_time(Time), floor(Time, WholeTime),EndTime is MT + WholeTime,
         asserta(end_time(EndTime))
    ; true),

    write('Number of new generations (-1 if not): '), read(NG),
    (NG \= -1 ->
        (retract(generations(_)); true), asserta(generations(NG))
    ; true),

    % Validation: At least end_time or generations must be provided
    (MT \= -1 ; NG \= -1) ->
        true
    ;
    write('Error: You must provide at least "Maximum duration" or "Number of new generations".'),
    nl, initialize_aditional_termination_params.
...
check_room_ocupancy([(Start,End,_)|Agenda], TotalOccupancy):-
    Time is End - Start,
...
generate(Room, Day):-
    ...
    length(SurgeriesList, Number),
    write('Room: '), write(Room), write('    |Number of Surgeries to Schedule: '), write(Number), nl,
    initialize_aditional_termination_params,
    initialize,!,
...
% the next generation
generate_generation(G,G,[Ind*V|Pop],Room,Day):-!,
        schedule_room_day(Ind,Room,Day),
	write('Generation '), write(G), write(':'), nl, write([Ind*V|Pop]), nl.
...
availability_all_surgeries([OpCode],Room,Day, TfinS):-
    ...
    insert_agenda_doctors((DTinS,DTfinS,OpCode),Day,LDoctors),
    insert_agenda_doctors((CTinS,CTfinS,OpCode),Day,LCleaners),
    insert_agenda_doctors((ATinS,ATfinS,OpCode),Day,LAnaesthesist).
